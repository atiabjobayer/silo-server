...
func (e *PlaybackProbeEnsurer) ensureProbeRepair(ctx context.Context, file *models.MediaFile) (*models.MediaFile, error) {
	...
	if NeedsCriticalProbeRepair(current) && ffprobePath != "" {
		repaired, err := e.ensureCriticalProbe(ctx, current, ffprobePath)
		if err != nil {
			return file, err
		}
		current = repaired
...
func (e *PlaybackProbeEnsurer) ensureStrmProbe(ctx context.Context, file *models.MediaFile) *models.MediaFile {
	ffprobePath := strings.TrimSpace(e.binaries().ffprobePath)
	if !strmNeedsRemoteProbe(file) || ffprobePath == "" {
		return file
	}
	updated, ok, err := repairStrmProbeData(ctx, ffprobePath, file, e.timeout)
...
func needsCopySafetyProbe(file *models.MediaFile) bool {
	...
	if file != nil && strings.EqualFold(filepath.Ext(file.FilePath), ".strm") {
		return false
	}
