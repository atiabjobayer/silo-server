...
describe("NotificationsAdminSettings", () => {
  ...
  it("registers Penguin Push Relay settings with the shared settings form", () => {
  ...
  it("shows the Penguin Push Relay channel status", async () => {
    useSettingsFormMock.mockReturnValue(makeForm());

    render(renderPage());

    expect(screen.getByText("Penguin Push Relay")).toBeInTheDocument();
    ...
    await userEvent.click(screen.getByRole("button", { name: /Penguin Push Relay/ }));
    ...
    expect(screen.getByText(/content-free request to Penguin's push relay/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/Penguin renews automatically/)).toBeInTheDocument();
